...
values(1, '2022-01-01'),
      ...
      (4, '2022-01-04');
------------------------------------------------------------------------------------------
ALTER TABLE storage.product_view ADD COLUMN customer_id int references storage.customers(ID);

UPDATE storage.product_view SET customer_id = 1 WHERE ID=1;
UPDATE storage.product_view SET customer_id = 1 WHERE ID=3;
UPDATE storage.product_view SET customer_id = 1 WHERE ID=4;
UPDATE storage.product_view SET customer_id = 1 WHERE ID=7;
UPDATE storage.product_view SET customer_id = 2 WHERE ID=2;
UPDATE storage.product_view SET customer_id = 3 WHERE ID=5;
UPDATE storage.product_view SET customer_id = 2 WHERE ID=6;
UPDATE storage.product_view SET customer_id = 2 WHERE ID=8;
UPDATE storage.product_view SET customer_id = 3 WHERE ID=9;

insert into storage.orders(customer_id, product_id, salespersons_id,quantity, unit_price, order_date)
values(2,3,1,1,10.00, '2023-03-15')
--#1
SELECT storage.orders.unit_price as the_highest_total_order_value, customers.name as customer_name, customers.email as customer_email
FROM storage.orders
...
from
    (SELECT
         COUNT(product_id) as quantity_view,
          product_id
...
select s.name,
     ...
     res.revenue
from
    (SELECT sum(unit_price) as revenue, salespersons_id
     FROM "storage".orders
...
from
 (SELECT product_id as id, storage.orders.quantity
 FROM storage.orders
 where date_trunc('MONTH',storage.orders.order_date ) = date_trunc('month', current_date - interval '1' month))
 as sold
...
FROM
    (SELECT
         SUM(storage.orders.unit_price) as orders_sum,
         customer_id,
         salespersons_id
     FROM storage.orders
...
from
	(select distinct count(storage.product_view.product_id) as viewed
...
from "storage".product
	inner join (select storage.product_view.product_id
				...
				from "storage".customers
					inner join "storage".product_view on storage.product_view.customer_id = storage.customers.id
				...
				having count(distinct storage.product_view.customer_id) = (select count(storage.customers.id)
						from "storage".customers)) as sss on storage.product.id = sss.product_id


--#9
select c."name", uni_orders
from
(select  count(res.order_month) as uni_orders
	,res.customer_id as customer
from
(SELECT distinct customer_id, date_trunc('MONTH',order_date) as order_month--, count( date_trunc('MONTH',order_date))
FROM "storage".orders
...
select s
from
(select salespersons_id, q_goods,count(customer_id)as customer_count
from
(SELECT customer_id, product_id, sum(quantity) as q_goods, salespersons_id
FROM "storage".orders
